...
## Later Card Product Decisions

- The product name and all visible app copy are English-only.
- The core mobile flow is intentionally single-screen: immediate card preview, hue control, phrase and text-color controls, background rail, PNG save, then share.
- The visual system is white with a saturated ocean-blue accent sampled from the vivid master background.
- The background rail contains one upload action plus five preset slots. Only Vivid and Pastel are populated for the first release; the remaining three slots stay visibly empty.
- PNG output is 1448 x 1086 (4:3), matching the generated master-image aspect ratio.
- The Some Time Later font is bundled locally under the SIL Open Font License and is reserved for the app title and generated card phrase.
